<template>
  <div class="player">
    <div class="blockTime">
      <el-date-picker v-model="value2" type="datetime" placeholder="选择日期时间" style="margin-top:10px;"></el-date-picker>
    ...
    <div class="videoParent">
      ...
      <div class="OperationLayer">
        <div class="jzprogressBar">
          <img src="../../../../../static/image/forward.png" @click="forwardclick"/>
          <div style="width:94%;margin-top:7px">
            <div ref="progress" style="height:5px;background:rgba(203,203,203,1);opacity:1;border-radius:10px;">
              <!-- 进度条 -->
              <div ref="bar" style="height:5px;background:rgba(0,126,255,1);opacity:1;border-radius:10px;"></div>
              <!-- 播放进度 -->
              <div ref="buffer" style="height:5px;background:#00f;" v-show="progressShow"></div>
            </div>
          </div>
          <img src="../../../../../static/image/backoff.png" @click="backoffclick"/>
        </div>
        <div ref="current" class="timeColor" v-show="progressShow"></div>
        <!-- 当前进度 -->
        <div ref="buffered" class="timeColor" v-show="progressShow"></div>
        <!-- 下载进度 秒 -->
        <div ref="duration" class="timeColor" v-show="progressShow"></div>
...
export default {
  ...
  methods: {
    ...
    changeProgress(e) {
      ...
      console.log((barLength / this.progress.clientWidth) * this.video.duration)
        console.log((barLength / this.progress.clientWidth) * 100 + "%")
        console.log(e.pageX - this.progress.offsetLeft,this.progress.clientWidth,this.video.duration)
    ...
    //快进
    backoffclick(){
        this.video.currentTime =this.video.currentTime*1.2
    },
    //快退
    forwardclick(){
        this.video.currentTime =this.video.currentTime/1.2
